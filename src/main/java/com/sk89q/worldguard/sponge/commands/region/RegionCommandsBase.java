...
import com.sk89q.worldguard.sponge.RegionContainer;
...
class RegionCommandsBase {
     ...
     */
    protected static RegionPermissionModel getPermissionModel(CommandSource sender) {
     ...
     */
    protected static World checkWorld(CommandContext args, CommandSource sender, char flag) throws CommandException {
    ...
    protected static ProtectedRegion checkRegionStandingIn(RegionManager regionManager, Player player, boolean allowGlobal) throws CommandException {
        ...
        if (set.size() == 0) {
            if (allowGlobal) {
                ...
                player.sendMessage(Texts.of(TextColors.GRAY, "You're not standing in any " +
     ...
     * @throws CommandException thrown on an error
     */
    protected static RegionSelector checkSelection(Player player) throws CommandException {
        SpongeWorldEdit worldEdit = WorldGuardPlugin.inst().getWorldEdit();
        RegionSelector selection = worldEdit.getRegionSelection(player);
        ...
        if (selection == null || !selection.isDefined()) {
            throw new CommandException(
                    "Please select an area first. " +
                            ...
                            "Note that currently you must use Forge WorldEdit.");
        }
    ...
    protected static ProtectedRegion checkRegionFromSelection(Player player, String id) throws CommandException {
        RegionSelector selection = checkSelection(player);

        // Detect the type of region from WorldEdit
        if (selection instanceof Polygonal2DRegionSelector) {
            Polygonal2DRegionSelector polySel = (Polygonal2DRegionSelector) selection;
            int minY = polySel.getIncompleteRegion().getMinimumY();
            int maxY = polySel.getIncompleteRegion().getMaximumY();
            return new ProtectedPolygonalRegion(id, polySel.getIncompleteRegion().getPoints(), minY, maxY);
        } else if (selection instanceof CuboidRegionSelector) {
            BlockVector min = selection.getIncompleteRegion().getMinimumPoint().toBlockVector();
            BlockVector max = selection.getIncompleteRegion().getMaximumPoint().toBlockVector();
     ...
     */
    protected static void warnAboutSaveFailures(CommandSource sender) {
        ...
        if (!failures.isEmpty()) {
            ...
            sender.sendMessage(Texts.of(TextColors.GOLD,
     ...
     */
    protected static void warnAboutDimensions(CommandSource sender, ProtectedRegion region) {
        int height = region.getMaximumPoint().getBlockY() - region.getMinimumPoint().getBlockY();
        if (height <= 2) {
            sender.sendMessage(Texts.of(TextColors.GRAY, "(Warning: The height of the region was " + (height + 1) + " block(s).)"));
        }
    }
     ...
     * @param region  the region
     */
    protected static void informNewUser(CommandSource sender, RegionManager manager, ProtectedRegion region) {
        if (manager.getRegions().size() <= 2) {
            sender.sendMessage(Texts.builder().append(
                    Texts.of(TextColors.GRAY, "(This region is NOW PROTECTED from modification from others. " +
                            "Don't want that? Use "),
                    Texts.of(TextColors.AQUA, "/rg flag " + region.getId() + " passthrough allow"),
                    Texts.of(TextColors.GRAY, ")")).build());
        }
    }
    ...
    protected static void setPlayerSelection(Player player, ProtectedRegion region) throws CommandException {
        ...
        if (region instanceof ProtectedCuboidRegion) {
            ...
            CuboidRegionSelector selection = new CuboidRegionSelector(worldEdit.getWorld(world), pt1, pt2);
            worldEdit.setSelection(player, selection);
            player.sendMessage(Texts.of(TextColors.YELLOW, "Region selected as a cuboid."));

        } else if (region instanceof ProtectedPolygonalRegion) {
            ProtectedPolygonalRegion poly2d = (ProtectedPolygonalRegion) region;
            Polygonal2DRegionSelector selection = new Polygonal2DRegionSelector(
                    worldEdit.getWorld(world), poly2d.getPoints(),
                    poly2d.getMinimumPoint().getBlockY(),
                    poly2d.getMaximumPoint().getBlockY() );
            worldEdit.setSelection(player, selection);
            player.sendMessage(Texts.of(TextColors.YELLOW, "Region selected as a polygon."));
     ...
     */
    protected static <V> void setFlag(ProtectedRegion region, Flag<V> flag, CommandSource sender, String value) throws InvalidFlagFormat {
